...
ArrayList<LayerInfo> createLayerInfo(int w, int h) {
  ...
  for (int i = 0; i< maxLayer; i++) {
    ...
    PShape shapes = getHoles(false, w, h, 2+ (maxLayer-i)/3, 20, minCircleDist);
...
void scaleLayerInfo(ArrayList<LayerInfo> layerInfos, float scaleFactor) {
  ...
  layerInfos.forEach( layerInfo -> {
    ...
    PGS_Conversion.getChildren(layerInfo.getShapes()).forEach( cell -> {

      cell.scale( scaleFactor);
    }
    );
...
void render(int w, int h, int dpos,
  ...
  for (int i = 0; i< layerInfos.size(); i++) {
    ...
    PShape shapeGroup = layerInfos.get(i).getShapes();
    ...
    if ( lastLayer != null) {
      ...
      //  for (PShape shape : shapeGroup.getChildren()) {
      //    shape.setFill(color(90), s*maxiterShadow);
      //    shape.setStroke(false);
      //  }

      //  shadowLayer.shape(shapeGroup, (maxiterShadow-s), 0);
      ...
      //  shadowLayer.shape(shapeGroup, (maxiterShadow+2-s), 0);
      //}
...
void highResRender() {
  ...
  highres.save("frames/" + name + " - "+ seed + " - "+iteration + (UHD ? " - UHD" : " - print") + ".png");
}
...
PShape getHoles(boolean getCircles, int w, int h, int minRadiusDiv, int maxRadiusDiv, int additionalCircleDist) {
  ...
  } else
  {
    ...
    Collection<PVector>  gridPoints = PGS_PointSet.nRooksLDS(-w/4, -h/4, w+(w/4), h+(h/4), floor(random(8, 15)));//plasticLDS haltonLDS(0, 0, width, height, 20);

    PShape voronoi = PGS_Voronoi.innerVoronoi(gridPoints, 1);

    blobs = new PShape(GROUP);

    var cells = PGS_Conversion.getChildren(voronoi);

    for (PShape cell : cells)
    {
      //cell = PGS_Morphology.round(cell, 0.2);
      cell = PGS_Contour.offsetCurvesInward(cell, PGS_Contour.OffsetStyle.ROUND, 9);

      // cell = PGS_Morphology.round (cell, 1);

      PShape ce = cell;
      int i = 0;
      for (PShape child : cell.getChildren()) {
        if ( i == 2) ce = PGS_Morphology.chaikinCut(child, 0.4, 4);// round(child,0.3);
        i++;
        ce.setFill(color(0));
        ce.setStroke(0);
      }
